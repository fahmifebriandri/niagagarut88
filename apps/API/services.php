...
*/	
...
else if($action == "loadDataProdukPager"){
	$data_user = $request['data_user'];
	$id_user = $data_user['id'];
	$created_by = $data_user['created_by'];
	$id_user_owner=($created_by == 1)?$id_user:$created_by;
	...
	*/
	...
	foreach($id_order_arr as $id_order){
		...
		$sql = "SELECT a.*, b.no_telepon as no_telepon_customer FROM tb_order a LEFT JOIN tb_customer b ON b.id = a.id_customer where a.id = '".$id_order."' ORDER BY a.id ASC;";
